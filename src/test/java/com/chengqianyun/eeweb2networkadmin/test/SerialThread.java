...
//  static String centerNo = "13800571500";   // 短信中心号码
  ...
  public void run() {
    ...
    String commName = list.get(list.size()-1);
    System.out.println("commName=" + commName);
    try {
        callPhone(commName, "15372095699");
...
//      try {
//        tmp = callPhone(tmp1, "15372095699");
//      } catch (Exception e) {
//        e.printStackTrace();
//      }
//      System.out.println("\n\n");
...
//    }
  ...
  public String callPhone(String commName, String phone) throws Exception {
...
//
      ...
      Thread.sleep(2 * 1000);
//      System.out.println("\n 准备发送短信:");
      flag = sendMessage(URLDecoder.decode(centerNo, "utf-8"), URLDecoder.decode("15372095699", "utf-8"), URLDecoder.decode("我是短信内容,发送试一下", "utf-8"), out, in);
//      flag = sendMessage(centerNo, phone, "博万通信", out, in);
   ...
   */
  public boolean sendMessage(String centerNo, String phoneNo, String message, OutputStream out, InputStream in) {
    boolean rsbool = true;
...
//         sendATCommand("AT\r");
   ...
   * @throws IOException
   */
  private void close(SerialPort serialPort) throws IOException {
    OutputStream out = serialPort.getOutputStream();
    InputStream in = serialPort.getInputStream();
    if (out != null) {
      try {
  ...
  private boolean sendCommand(InputStream in, OutputStream out, String content) throws IOException, InterruptedException {
    write(content, out);
